//------------------------------------------------------------------------------
// LG_BreadthFirstSearch_vanilla_template:  BFS using only GraphBLAS API
//------------------------------------------------------------------------------
    ...
    do
    {
        ...
        #ifdef LG_BFS_EXTENDED
        {
            if (max_level >= 0 && current_level >= max_level)
            {
                ...
                // this will halt with just the src node.  Note that
                // current_level has not yet been advanced, so the test is
                // "current_level >= max_level".  See the SSGrB version,
                // where the test is done just after the current level (k)
                // has been advanced.
                break ;
            }
            if (dest >= 0)
            {
                ...
                // if src == dest.
                int64_t x ; // value is unused
                GrB_Info info = GrB_Vector_extractElement (&x, mask, dest) ;
                if (info == GrB_SUCCESS)
                {
                    break ;
                }
