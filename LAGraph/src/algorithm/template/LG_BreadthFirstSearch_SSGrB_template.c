//------------------------------------------------------------------------------
// LG_BreadthFirstSearch_SSGrB_template:  BFS using Suitesparse extensions
//------------------------------------------------------------------------------
...
#endif
{
    ...
    for (int64_t nvisited = 1, k = 1 ; nvisited < n ; nvisited += nq, k++)
    {
        ...
        //----------------------------------------------------------------------
        // check for early termination (extended version of the algorithm)
        //----------------------------------------------------------------------
        ...
        #ifdef LG_BFS_EXTENDED
        {
            ...
            {
                // halt if max_level has been reached; if max_level is zero,
                // this will halt with just the src node.  Note that k starts
                // at 1, and has just been advanced for subsequent iterations,
                // so k-1 is the last level added to pi and/or v.  So the test
                // for the max level is "k > max_level".  This differs from
                // the vanilla version (in that version current_level has not
                // been advanced yet, when the test is made).
                break ;
            }
            if (dest >= 0)
            {
                // halt if dest node has been found; this will stop at level 0
                // if src == dest.  The mask is either pi or v, and is always
                // held in bitmap/full format, so this takes O(1) time.
